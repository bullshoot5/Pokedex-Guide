...
//olay izleyicisi ile searc butonuna active clası ekleniyor
searchBtn.addEventListener('click', () => {
  search.classList.toggle(`active`);
});
//arama için olzy izleyicisi eklendi
...
//API alınan pokemonlar oluşturulan divlere basılarak ekrana çıkartılıyor.
const createPokemonCard = (pokemon) => {
  const pokemonDiv = document.createElement('div');
  pokemonDiv.classList.add('pokemon');
  //json veri içerisinde id yakalar
  const pokemonId = pokemon.id.toString().padStart(3, '0'); //padStart: belirlenen basamak değerlenirin ne ile tamamlanması gerektiğini bildirir.(3 basamağı sıfır ile doldurur)
  //json veri içerisinde type yakalar
  const pokemonType = pokemon.types[0].type.name;
  //json veri içerisinde arka planı yakalar
  const pokemonBg = bg_color[pokemonType];
  //json veri içerisinde alına type  göre yukaıda tanmlanan bgcolor göre arka planı değiştirir
